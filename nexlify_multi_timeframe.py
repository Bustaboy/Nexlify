...
class MultiTimeframeAnalyzer:
    ...
    def __init__(self, config: Dict = None):
        ...
        logger.info(f"📊 Multi-Timeframe Analyzer initialized with {len(self.timeframes)} timeframes")
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        # Moving averages
        sma_20 = pd.Series(close).rolling(20).mean().iloc[-1] if len(close) >= 20 else close[-1]
        sma_50 = pd.Series(close).rolling(50).mean().iloc[-1] if len(close) >= 50 else close[-1]
        ...
        bb_upper = (sma_20_series + 2 * std_20).iloc[-1] if len(close) >= 20 else close[-1]
        bb_lower = (sma_20_series - 2 * std_20).iloc[-1] if len(close) >= 20 else close[-1]
    ...
    def _generate_recommendation(self, signal: TimeframeSignal, confluence: float) -> str:
