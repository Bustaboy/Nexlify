...
class FeatureEngineer:
    ...
    def _add_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [14, 20]:
            ...
            df[f'mfi_{period}'] = 100 - (100 / (1 + positive_flow / (negative_flow + 1e-10)))
    ...
    def _add_pattern_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [20, 50]:
            ...
            df[f'broke_resistance_{period}'] = (df['close'] > df[f'resistance_{period}'].shift()).astype(int)
            df[f'broke_support_{period}'] = (df['close'] < df[f'support_{period}'].shift()).astype(int)

        # Trend detection
        sma_20 = df['close'].rolling(20).mean()
        sma_50 = df['close'].rolling(50).mean()
        df['golden_cross'] = ((sma_20 > sma_50) & (sma_20.shift() <= sma_50.shift())).astype(int)
        df['death_cross'] = ((sma_20 < sma_50) & (sma_20.shift() >= sma_50.shift())).astype(int)
    ...
    def _add_microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'volume' in df.columns:
            df['amihud_illiquidity'] = abs(df['close'].pct_change()) / (df['volume'] + 1e-10)

        # Price efficiency (autocorrelation)
        for lag in [1, 5, 10]:
            df[f'autocorr_{lag}'] = df['close'].pct_change().rolling(20).apply(
        ...
        for period in [30]:
            df[f'hurst_{period}'] = df['close'].rolling(period).apply(
                lambda x: self._calculate_hurst(x) if len(x) == period else 0.5
            )
    ...
    def get_feature_importance_groups(self) -> Dict[str, List[str]]:
        ...
        return {
            'price': [col for col in self.feature_names if any(x in col for x in ['return', 'momentum', 'price_position'])],
            'technical': [col for col in self.feature_names if any(x in col for x in ['sma', 'ema', 'rsi', 'macd', 'bb_'])],
            'volatility': [col for col in self.feature_names if 'vol' in col or 'atr' in col],
            'volume': [col for col in self.feature_names if 'volume' in col or 'obv' in col or 'mfi' in col],
