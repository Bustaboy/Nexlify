    ...
    def check_session(self):
        ...
        try:
            ...
            is_valid = self.security_manager.validate_session(
                self.session_token, self.get_current_ip()
            )
...
class CyberGUI(QMainWindow):
    ...
    def _init_components(self):
        ...
        try:
            ...
            self.security_manager = SecurityManager(
                master_password=self.app_config.get("security", {}).get(
                    "master_password", ""
                )
            )
    ...
    def _show_login_dialog(self):
        ...
        title.setStyleSheet(
            f"color: {self.config.accent_primary}; font-size: 18px; font-weight: bold;"
        )
        ...
        has_master_password = self.app_config.get("security", {}).get(
    ...
    def _authenticate(self, dialog: QDialog, password: str, twofa_code: Optional[str]):
        ...
        try:
            # For simple PIN mode
            if not self.app_config.get("security", {}).get(
                "master_password_enabled", False
            ):
            ...
            if not session:
                QMessageBox.warning(
                    dialog, "Authentication Failed", "Invalid credentials"
                )
                return
            ...
            if self.app_config.get("security", {}).get("2fa_enabled", False):
                ...
                if not self.security_manager.two_factor_auth.verify_token(
                    username, twofa_code
                ):
    ...
    def _validate_btc_address(self, address: str):
        ...
        if is_valid:
            ...
            self.btc_address_valid_label.setStyleSheet(
                f"color: {self.config.accent_success};"
            )
            self.btc_address_input.setStyleSheet(
                f"border: 1px solid {self.config.accent_success};"
            )
            ...
            self.btc_address_valid_label.setStyleSheet(
                f"color: {self.config.accent_error};"
            )
            self.btc_address_input.setStyleSheet(
                f"border: 1px solid {self.config.accent_error};"
            )
        ...
        try:
            ...
            try:
                ...
                balance = await asyncio.wait_for(
                    test_exchange.fetch_balance(), timeout=10.0
                )
                ...
                self.log_widget.append_log(
                    f"Exchange connection test passed: {exchange}", "INFO"
                )

                # Log test
                await self.audit_manager.audit_system_event(
                    "exchange_test", "info", {"exchange": exchange, "success": True}
                )
                ...
                self.log_widget.append_log(
                    f"Exchange connection timeout: {exchange}", "WARNING"
                )
                ...
                self.log_widget.append_log(
                    f"Exchange connection failed: {exchange} - {str(e)}", "ERROR"
                )

        except Exception as e:
            error_handler.log_error(
                e, {"method": "_test_exchange_connection", "exchange": exchange}
            )
    ...
    def _execute_trade(self, side: str):
        ...
        if "/" not in pair:
            QMessageBox.warning(
                self, "Invalid Pair", "Please enter a valid trading pair"
            )
            return
        ...
        if reply == QMessageBox.Yes:
            # Execute trade
            asyncio.create_task(
                self._execute_trade_async(pair, side, amount, order_type, price)
            )
    ...
    def _withdraw_funds(self):
        ...
        if not CryptoUtils.validate_address(btc_address, "BTC"):
            QMessageBox.warning(
                self, "Invalid Address", "Please enter a valid Bitcoin address"
            )
            return
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Invalid Amount", "Please enter a valid withdrawal amount"
            )
            return

        # Check minimum withdrawal
        min_withdrawal = self.app_config.get("withdrawal", {}).get(
            "min_withdrawal_usdt", 100
        )
        if amount < min_withdrawal:
            QMessageBox.warning(
                self, "Amount Too Low", f"Minimum withdrawal is ${min_withdrawal} USDT"
            )
            return
        ...
        try:
            btc_price = (
                self.neural_net.btc_price
                if self.neural_net and self.neural_net.btc_price > 0
                else 45000
            )
        except:
    ...
    def _save_settings(self):
        ...
        try:
            ...
            for exchange, inputs in self.api_inputs.items():
                ...
                if api_key and secret:
                    ...
                    if hasattr(self.security_manager, "encryption_manager"):
                        encrypted_key = (
                            self.security_manager.encryption_manager.encrypt_data(
                                api_key
                            )
                        )
                        encrypted_secret = (
            ...
            QMessageBox.information(
                self, "Settings Saved", "All settings have been saved successfully."
            )

        except Exception as e:
            error_handler.log_error(e, {"method": "_save_settings"})
            QMessageBox.critical(
                self, "Save Failed", f"Failed to save settings: {str(e)}"
            )
    ...
    def _setup_2fa(self):
        ...
        if not self.security_manager or not hasattr(
            self.security_manager, "two_factor_auth"
        ):
            QMessageBox.warning(
                self,
                "2FA Not Available",
        ...
        try:
            ...
            if not qr_data:
                QMessageBox.warning(
                    self, "2FA Setup Failed", "Failed to generate 2FA QR code"
                )
                return
            ...
            QMessageBox.critical(
                self, "2FA Setup Failed", f"Failed to setup 2FA: {str(e)}"
            )
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.master_password_check.setChecked(
                self.app_config.get("security", {}).get(
                    "master_password_enabled", False
                )
            )
        ...
        try:
            ...
            try:
                if self.neural_net and hasattr(self.neural_net, "total_profit"):
                    ...
                    self.profit_series.replace(
                        current_hour, 0, current_hour, self.neural_net.total_profit
                    )
    ...
    async def _refresh_positions(self):
        ...
        try:
            ...
            if positions_data:
                ...
                self.log_widget.append_log(
                    f"Refreshed {len(positions)} open positions", "INFO"
                )
            else:
            ...
            self.log_widget.append_log(
                f"Failed to refresh positions: {str(e)}", "ERROR"
            )
    ...
    def _enable_strategy(self):
        ...
        try:
            ...
            if not selected_items:
                QMessageBox.warning(
                    self, "No Selection", "Please select a strategy to enable"
                )
                return

            selected_text = selected_items[0].text()
            strategy_name = (
                selected_text.split(" - ")[0].strip().lower().replace(" ", "_")
            )
            ...
            if self.strategy_optimizer:
                ...
                if success:
                    ...
                    selected_items[0].setText(
                        f"{selected_text.split(' - ')[0]} - Active"
                    )
                    selected_items[0].setForeground(QColor(self.config.accent_success))
                    self.log_widget.append_log(
                        f"Strategy enabled: {strategy_name}", "INFO"
                    )
                    QMessageBox.information(
                        self,
                        "Strategy Enabled",
                    ...
                    QMessageBox.warning(
                        self, "Error", f"Strategy not found: {strategy_name}"
                    )
            else:
                QMessageBox.warning(
                    self, "Not Available", "Strategy optimizer not initialized"
                )
    ...
    def _disable_strategy(self):
        ...
        try:
            ...
            if not selected_items:
                QMessageBox.warning(
                    self, "No Selection", "Please select a strategy to disable"
                )
                return
            ...
            if reply == QMessageBox.Yes:
                ...
                if self.strategy_optimizer:
                    ...
                    if success:
                        # Update UI
                        selected_items[0].setText(
                            f"{selected_text.split(' - ')[0]} - Inactive"
                        )
                        selected_items[0].setForeground(QColor(self.config.text_dim))
                        self.log_widget.append_log(
                            f"Strategy disabled: {strategy_name}", "WARNING"
                        )
                        QMessageBox.information(
                            self,
                            "Strategy Disabled",
    ...
    def _configure_strategy(self):
        ...
        try:
            ...
            if not selected_items:
                QMessageBox.warning(
                    self, "No Selection", "Please select a strategy to configure"
                )
                return
            ...
            save_btn.clicked.connect(
                lambda: self._save_strategy_config(dialog, strategy_name)
            )
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to open configuration: {str(e)}"
            )
    ...
    def _save_strategy_config(self, dialog, strategy_name):
        ...
        try:
            self.log_widget.append_log(
                f"Strategy configuration saved: {strategy_name}", "INFO"
            )
            QMessageBox.information(
                self,
                "Configuration Saved",
    ...
    def _handle_session_expired(self):
        ...
        self.is_authenticated = False
        QMessageBox.warning(
            self, "Session Expired", "Your session has expired. Please login again."
        )
        self._show_login_dialog()
