...
class PredictiveEngine:
    ...
    def predict_price(self, symbol: str, current_price: float,
    ...
    def detect_patterns(self, symbol: str, prices: List[float]) -> List[Dict]:
        ...
        try:
            ...
            df['sma_50'] = df['price'].rolling(window=50).mean() if len(prices) >= 50 else df['price'].rolling(window=20).mean()
    ...
    def calculate_indicators(self, prices: List[float], volumes: List[float] = None) -> Dict:
        """
    ...
    def score_trade_opportunity(self, symbol: str, prediction: Dict,
                               indicators: Dict, patterns: List[Dict]) -> Dict:
        """
        ...
        try:
            ...
            if prediction['direction'] == 'bullish':
                ...
                reasons.append(f"Bullish prediction (+{prediction['change_percent']:.2f}%)")
            elif prediction['direction'] == 'bearish':
                score -= prediction['confidence'] * 0.3
                reasons.append(f"Bearish prediction ({prediction['change_percent']:.2f}%)")
