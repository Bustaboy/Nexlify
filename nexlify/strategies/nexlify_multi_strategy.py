...
class MultiStrategyOptimizer:
    ...
    def _initialize_strategies(self):
        ...
        try:
            ...
            error_handler.log_error(e, "Strategy initialization failed", severity="error")
        ...
        for name, strategy in self.strategies.items():
            if strategy.metrics.total_trades >= 10:
                if strategy.metrics.win_rate < 0.3:
                    logger.warning(f"Strategy {name} has low win rate, consider disabling")
                if strategy.metrics.profit_factor < 0.5:
                    logger.warning(f"Strategy {name} has poor profit factor, consider disabling")
