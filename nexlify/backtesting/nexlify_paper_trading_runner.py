...
class PaperTradingRunner:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "training": {"episodes": 100, "max_steps": 1000, "save_frequency": 10},
            "agents": [
                {
            ...
            "logging": {"level": "INFO", "file": "paper_trading/logs/session.log"},
        }
    ...
    async def train_agent(self, agent_type: str = "adaptive", episodes: int = 100):
        ...
        for episode in range(1, episodes + 1):
            ...
            if episode % save_frequency == 0:
                checkpoint_path = (
                    f"models/paper_trading_{agent_type}_episode_{episode}.pt"
                )
    ...
    async def run_multi_agent_session(self, duration_hours: Optional[float] = None):
        ...
        for agent_config_dict in self.config.get("agents", []):
            ...
            if agent_instance:
                self.orchestrator.load_agent_model(
                    agent_config.agent_id, agent_instance
                )
    ...
    async def evaluate_agents(self, model_paths: List[str], episodes: int = 10):
        ...
        for model_path in model_paths:
            ...
            # Load agent
            agent = create_optimized_agent(
                state_size=env.state_size, action_size=env.action_size, auto_detect=True
            )
            ...
            for episode in range(episodes):
                ...
                logger.debug(
                    f"  Episode {episode + 1}: Return={episode_stats.total_return_percent:.2f}%"
                )
            ...
            logger.info(
                f"  Mean Return: {results[model_path]['mean_return']:.2f}% ± {results[model_path]['std_return']:.2f}%"
            )
            logger.info(f"  Mean Win Rate: {results[model_path]['mean_win_rate']:.1f}%")
            logger.info(
                f"  Best: {results[model_path]['best_return']:.2f}%, Worst: {results[model_path]['worst_return']:.2f}%"
            )
        ...
        sorted_results = sorted(
            results.items(), key=lambda x: x[1]["mean_return"], reverse=True
        )
...
def main():
    ...
    parser.add_argument(
        "mode", choices=["train", "evaluate", "multi-agent"], help="Operation mode"
    )
    ...
    parser.add_argument("--models", nargs="+", help="Model paths for evaluation")
    ...
    if args.mode == "train":
        asyncio.run(
            runner.train_agent(agent_type=args.agent_type, episodes=args.episodes)
        )
        ...
        asyncio.run(
            runner.evaluate_agents(model_paths=args.models, episodes=args.episodes)
        )
