...
class NightCityErrorHandler:
    ...
    def log_fatal_error(self, exc_type, exc_value, exc_traceback, thread_info: str = ""):
        ...
        # Save crash report
        crash_file = self.crash_report_path / f"crash_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_crash_report(self, exc_type, exc_value, exc_traceback, thread_info: str = "") -> Dict:
            ...
            safe_config = {
                ...
                'environment': {
                    ...
                    'notifications_enabled': bool(
                        self.config.get('environment', {}).get('emergency_contact') or
                        self.config.get('environment', {}).get('telegram_bot_token')
                    )
                }
            }
    ...
    def _notify_fatal_crash(self, crash_report: Dict):
        ...
        if env_config.get('emergency_contact'):
            self._send_email_notification(
                "FATAL CRASH - Nexlify",
                json.dumps(crash_report, indent=2)
            )
